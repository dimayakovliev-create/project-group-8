...
class AddressBook(UserDict):
    ...
    def change_email_in_record(self, name, old_email, new_email):
        ...
        if not hasattr(record, 'emails') or record.emails is None:
            return f"No emails found for contact {name}."
        
        try:
            record.edit_email(old_email, new_email)
            save_data(self)  # HW08! (правки) Збереження даних після зміни email
            return f"Email '{old_email}' successfully changed to '{new_email}' for {name}."
        except ValueError as e:
            return str(e)
...
def main():
    ...
    while True:
        ...
        if need_update_completer:
            phone_hint = {"<phone 10 digits: CCCNNNNNNN>": None}
        ...
        # Створюємо словник для NestedCompleter, де ключі — це команди, а значення — словник з іменами контактів для відповідних команд.
            completer_dictionary = {
                ...
                "change-phone": contacts_with_existing_phones,       # Підскаже імена існуючих контактів
                "change-email": contacts_with_existing_emails, # Підскаже ім'я для зміни email
        ...
        if command in ["close", "exit"]:
            ...
            print("Invalid command. Enter the argument for the command: add-phone, change-phone, change-email, delete, phone, all, add-birthday, add-email, search, show-birthday, birthdays, exit, close, hello")
